...
UserSchema.methods.createAccessToken = function (uuid: string): string {
    const accessToken = jwt.sign({ _id: this._id, email: this.email, username: this.username }, process.env.ACCESS_TOKEN_SECRET, {
...
UserSchema.methods.createRefreshToken = function (uuid: string): string {
    const refreshToken = jwt.sign({ _id: this._id, email: this.email, username: this.username }, process.env.REFRESH_TOKEN_SECRET, {
