...
export default function startServer(expressApplication: Express) {
        ...
        expressApplication.post('/', (_, res) => res.status(200).json({ message: "Ping"}));

        const serverPort = process.env.SERVER_PORT || process.env.PORT; 
        http.createServer(expressApplication).listen(serverPort, () => { console.log(`Server is running at ${serverPort}`)});
}